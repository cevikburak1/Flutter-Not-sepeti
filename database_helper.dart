...
class DatabaseHelper{
  ...
  Future<Database> _intializeDatabase() async {
    var lock = Lock();
    Database _db;
    ...
    if (_db == null) {
      await lock.synchronized(() async {
        if (_db == null) {
          ...
          print("OLUŞACAK DB NİN PATHİ $path");
          var file = new File(path);

          // check if file exists
          if (!await file.exists()) {
            // Copy from asset
            ByteData data = await rootBundle.load(join("assets", "sql.db"));
            List<int> bytes =
 ...
 Future<List<Map<String,dynamic>>> kategorileriGetir() async{
  ...
  Future<List<Kategori>> kategoriListesiniGetir() async{
    ...
    var katagoriIcerenMapListesi = await kategorileriGetir();
    var kategoriListesi =List<Kategori>();
    for(Map map in katagoriIcerenMapListesi){
  ...
  Future<List<Map<String,dynamic>>> notlariGetir() async{
    ...
    var db=await _getDatabase();
    var sonuc= await db.rawQuery('select * from "not" inner join kategori on kategori.kategoriID = "not".kategoriID;');
    return sonuc;
  }
